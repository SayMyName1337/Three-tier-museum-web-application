    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
        ...
        <div class="collapse navbar-collapse">
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="/exhibitions/select-ao-to-attach?ex-id=${exhibition.id}">
                        Привязка произведения искусства к выставке «<c:out value="${exhibition.name}"/>»
                    </a>
                </li>
            </ul>
        </div>
        ...
        <div class="table-responsive">
            <c:choose>
                <c:when test="${exhibitedArtObjects.size() == 0}">
                    <h3 class="text-center">Произведения искусства, которые принимают участие в выставке "<c:out value='${exhibition.name}'/>" не найдены</h3>
                </c:when>
                <c:otherwise>
                    <table class="table table-striped table-bordered">
                        <caption><h2 class="text-center">Произведения искусства, которые принимают участие в выставке "<c:out value='${exhibition.name}'/>"</h2></caption>
                        ...
                        <tbody>
                            <c:forEach var="artObject" items="${exhibitedArtObjects}">
                                <tr>
                                    ...
                                    <td class="actions">
                                        <a href="/exhibitions/remove-ao-from-ex?ex-id=${exhibition.id}&ao-id=<c:out value='${artObject.id}' />" class="btn btn-danger btn-sm">Убрать с выставки</a>
                                    </td>
                                </tr>
                            </c:forEach>
                        </tbody>
